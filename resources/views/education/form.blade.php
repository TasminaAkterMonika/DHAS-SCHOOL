...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'education?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="col-md-12">
				...
				{!! Form::hidden('id', $row['id']) !!}					
						...
						<div class="icon-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["icon"],"/uploads/images/icon") !!}
						</div>
						     ...
						     class='form-control form-control-sm ' /> 
									  ...
									  </div> {!! Form::hidden('details', $row['details']) !!}					
									  <div class="form-group row  " >
										<label for="Id2" class=" control-label col-md-4 text-left"> Id2 </label>
										<div class="col-md-6">
										  <input  type='text' name='id2' id='id2' value='{{ $row['id2'] }}' 
						     class='form-control form-control-sm ' /> 
										 </div> 
										 <div class="col-md-2">
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#status").jCombo("{!! url('education/comboselect?filter=status:id:title') !!}",
		{  selected_value : '{{ $row["status"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("education/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
